...
class RectangularRoom(object):
    ...
    def clean_tile_at_position(self, pos, capacity):
        ...
        """
        self.tiles[(math.floor(pos.get_x()), math.floor(pos.get_y()))] = max(self.tiles[(math.floor(pos.get_x()), math.floor(pos.get_y()))] - capacity, 0)
    ...
    def is_position_in_room(self, pos):
        ...
        """
        return pos.get_x() <= self.width and pos.get_x() >= 0 and pos.get_y() <= self.height and pos.get_y() >= 0
...
class StandardRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        if self.room.is_position_valid(new_position):
            # print(new_position.get_x(), new_position.get_y(), self.room.is_position_valid(new_position))
...
def run_simulation(num_robots, speed, capacity, width, height, dirt_amount, min_coverage, num_trials,
    ...
    for i in range(num_trials):
        ...
        for j in range(num_robots):
            ...
            robots.append(robot)
        while((float(room.get_num_cleaned_tiles())/float(room.get_num_tiles())) < min_coverage):
            timestep += 1
            for robot in robots:
